      ...
      if (counter >= 50) { // main exit loop command
       ...
       for(int i=0; i<240; i++){
          ...
          if(c<200){ // 320/2 ie. if (c < half)
           ...
           whiteTotal = c; //add the position of the white pixels (if its white)
       }
       ...
       propSignal = whiteTotal*kp;
        
        derivativeSignal = ((whiteTotal-prev_error)/0.1)*kd;
        ...
        if(110 < whiteTotal < 130){
          rightMotor = 70.0;
        	leftMotor = -70;//negative so motors turn in the same direction
        }else{
